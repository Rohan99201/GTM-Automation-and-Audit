...
// ── Update helpers ─────────────────────────────────────────────────────────
export const updateTag = (token, entityPath, body) =>
  gtmFetch(token, `/${entityPath}`, { method: "PUT", body: JSON.stringify(body) });

export const updateTrigger = (token, entityPath, body) =>
  gtmFetch(token, `/${entityPath}`, { method: "PUT", body: JSON.stringify(body) });

export const updateVariable = (token, entityPath, body) =>
  gtmFetch(token, `/${entityPath}`, { method: "PUT", body: JSON.stringify(body) });
...
export function parseDataLayerDoc(rows) {
  ...
  rows.forEach((row) => {
    ...
    if (!events[eventName]) {
      events[eventName] = {
        eventName, variables: [], tagType: "", measurementId: "",
        sendEcommerce: ecommerceEvents.some((e) => eventName.toLowerCase().includes(e)),
      };
    }
...
export function buildPayloadsFromEvent(event) {
  const { eventName, variables, tagType, measurementId, sendEcommerce } = event;
  ...
  // 3. Tag — only if tagType specified in the doc
  if (tagType) {
    const resolvedType = tagType === "ga4" ? "gaawe" : tagType === "google tag" ? "googtag" : tagType;
    let tagParams = [];

    if (resolvedType === "gaawe") {
      tagParams = [
        { type: "template", key: "measurementIdOverride", value: measurementId || "{{GA4 Measurement ID}}" },
        { type: "template", key: "eventName",             value: eventName },
      ];
      if (sendEcommerce) {
        tagParams.push({ type: "boolean",  key: "sendEcommerceData",  value: "true" });
        tagParams.push({ type: "template", key: "ecommerceMacroData", value: "{{dlv - ecommerce}}" });
      }
    } else if (resolvedType === "googtag") {
      tagParams = [{ type: "template", key: "tagId", value: measurementId || "{{GA4 Measurement ID}}" }];
    } else if (resolvedType === "ua") {
      tagParams = [
        { type: "template", key: "trackingId", value: measurementId || "{{UA Tracking ID}}" },
        { type: "template", key: "trackType",  value: "TRACK_EVENT" },
      ];
    }

    const tagLabel = resolvedType === "gaawe" ? "GA4" : resolvedType === "googtag" ? "GTag" : resolvedType.toUpperCase();
    payloads.tags.push({
      name: `${tagLabel} - ${eventName}`,
      type: resolvedType,
      parameter: tagParams,
      // firingTriggerId injected after trigger is created in route.js
    });
  }
